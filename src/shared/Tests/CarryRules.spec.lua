...
return function(CarryRules)
    ...
    describe("carryEfficiency", function()
        it("demanda cubierta → sin penalización", function()
            expect(CarryRules.carryEfficiency(1, 1)).to.equal(1)
            expect(CarryRules.carryEfficiency(2, 2)).to.equal(1)
        end)
        it("large en solitario → degrada a la razón, no bloquea (D8)", function()
            expect(CarryRules.carryEfficiency(2, 1)).to.equal(0.5)
        end)
        it("con al menos un cargador el factor NUNCA es 0: bloquear iniciar está prohibido", function()
            expect(CarryRules.carryEfficiency(10, 1) > 0).to.equal(true)
        end)
        it("cargadores de sobra no superan 1", function()
            expect(CarryRules.carryEfficiency(2, 5)).to.equal(1)
        end)
        it("demanda inválida → sin penalización", function()
            expect(CarryRules.carryEfficiency(0, 1)).to.equal(1)
            expect(CarryRules.carryEfficiency(nil, 1)).to.equal(1)
        end)
    end)
